function [  unused_cores, pairing_matrix, FA, Undetected, Detected] = result_valoration( SamplesGT , SamplesExperiment, threshold )
...
% 
% selected_Sample = pairing_matrix(i,4);
% minDist2Sample = pairing_matrix(i,3);
% % reset value
% 
% % IMPLIED THAT minDist2Sample < threshold
% 
%     % case 1: we paired a core-monument and is under ther optimal threshold
%     if ((selected_Sample ~= 0) && (minDist2Sample < suboptimal_threshold)) 
%         % 100% modificator -> max=1, min = 0.5
% %         modificator=1;
%         evaluation = evaluation + 1*(0.5+(1-minDist2Sample/threshold)*0.5);
%     % case 2: suboptimal case -> max=0.5, min = 0
%     elseif ((selected_Sample ~= 0) && (minDist2Sample < threshold) && (minDist2Sample > suboptimal_threshold))    
%         % 50% modificator
% %         modificator=0.5;
%         evaluation = evaluation + 0.5*(1-minDist2Sample/threshold);
%         
